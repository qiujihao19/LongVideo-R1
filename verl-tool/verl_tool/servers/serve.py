...
class AsyncToolManager:
    ...
    def __init__(
        ...
        tool_init_kwargs: Optional[Dict[str, Dict[str, Any]]] = None,
    ):
        self.tools: Dict[str, Any] = {}
        self.use_tqdm = use_tqdm
        self.done_if_invalid = done_if_invalid
        self.config = config
        self.tool_init_kwargs = tool_init_kwargs or {}
    ...
    def _initialize_tools(self, tool_types: Tuple[str]) -> None:
        ...
        for tool_type in tool_types:
            try:
                ...
                init_kwargs = self.tool_init_kwargs.get(tool_type, {})
    ...
    def __init__(
        ...
        if use_ray:
            ...
            self.tool_manager = RayToolManager(
                ...
                use_tqdm,
                done_if_invalid,
                tool_init_kwargs=tool_init_kwargs,
            )
        else:
...
def main(
    ...
    tool_init_kwargs: Dict[str, Dict[str, Any]] = {}
    if tool_config_path:
        with open(tool_config_path, "r", encoding="utf-8") as f:
            loaded = json.load(f)
        if not isinstance(loaded, dict):
            raise ValueError("tool_config_path must point to a JSON object.")
        tool_init_kwargs = loaded
